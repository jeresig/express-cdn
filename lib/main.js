...
var renderTag = function(options, assets, attributes) {
  ...
  if (options.production) {
    ...
    if (typeof assets === 'object') {
      ...
      return createTag(src, "/" + name, attributes) + endline;
    } else {
      timestamp = fs.statSync(path.join(options.publicDir, assets)).mtime.getTime();
      position = assets.lastIndexOf('.');
      return createTag(src, _(assets).splice(position, 0, '.' + timestamp), attributes) + endline;
    }
  } else {
    ...
    if (typeof assets === 'object') {
      ...
      return createTag(src, assets, attributes, version) + endline;
    } else {
  ...
  if (options.production) {
    var walker = function () {
      ...
      walker.on('file', function(root, stat, next) {
        ...
        if (ext === '.jade' || ext === '.ejs' || ext === options.ext) {
